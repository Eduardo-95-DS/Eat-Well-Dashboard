...
city = st.sidebar.selectbox('', cities, placeholder="Choose an option")
...
with st.container():
    ...
    with col1:
        ...
        df1=(df1[['restaurant_name','aggregate_rating','average_cost_for_two','price_range','country','city']]
                .sort_values('aggregate_rating',ascending=False)
                .rename(columns={'restaurant_name':'restaurant','aggregate_rating':'rating','average_cost_for_two':'cost_for_two_$','price_range':'price'})
...
with st.container():
    ...
    st.markdown("<h4 style='text-align:center;margin-top: 0px;'>Most voted</h4>",unsafe_allow_html=True)

    df1=(df[['restaurant_name','country','votes']].groupby(['restaurant_name','country'])
                                                  .sum()
                                                  .sort_values('votes',ascending=False)
                                                  .reset_index()
                                                  .head(10))
    
    fig = px.bar(df1, x='votes', y="restaurant_name", color='country',text_auto=True,
